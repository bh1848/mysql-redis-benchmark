...
본 레포지토리는 관계형 데이터베이스(RDBMS, MySQL)와 인메모리 기반 NoSQL(Redis)의 데이터 처리 성능을 비교하기 위해 작성된 벤치마크 프로젝트입니다.
동일한 하드웨어 및 애플리케이션 로직 하에서 저장소 구조(Disk vs Memory)가 실제 CRUD Latency에 미치는 영향을 분석했습니다.
...
* 하위 구현체는 순수 DB 연산 로직만 구현하도록 강제하여 측정 오차를 최소화했습니다.
