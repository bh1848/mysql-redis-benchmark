...
- **문제**: 반복적인 벤치마크 실행 시 이전 데이터가 남아 PK 충돌(`Duplicate entry`)이 발생하고, 테스트의 멱등성이 깨지는 현상 확인.
- **해결**: 인덱스 파편화를 유발하는 `DELETE` 쿼리 대신, JPA의 `ddl-auto: create` 옵션을 사용해 앱 구동 시마다 스키마를 새로 생성하도록 설정.
- **결과**: 데이터 간섭 없는 클린한 상태에서 자동화 테스트를 수행하고, 인덱스 노이즈가 제거된 순수 성능 데이터를 확보.
- [👉 포스트 보러가기](https://hzeror.netlify.app/MySQL-Redis-benchmarks-ddl-auto/)


### 2) 시스템 타이머 해상도 한계 극복 (평균 역산법)
- **문제**: Windows 환경에서 `currentTimeMillis()`의 낮은 정밀도(10~15ms)로 인해, 마이크로초 단위인 Redis 응답 시간이 0ms로 측정되는 문제 발생.
- **해결**: 단건 측정 방식에서 벗어나, 1,000건의 배치(Batch) 단위 총 소요 시간을 측정한 뒤 이를 평균값으로 나누어 역산하는 방식으로 개선.
- **결과**: 숨겨져 있던 Redis의 실제 평균 응답 속도인 0.17ms를 정확히 측정하여 데이터 신뢰성을 확보.
- [👉 포스트 보러가기](https://hzeror.netlify.app/MySQL-Redis-benchmarks-precise-execution-time-measurement/)

### 3) 네트워크 RTT 기반의 병목 구간 분석
- **문제**: Redis 서버 리소스가 충분함에도 처리량(OPS)이 이론적 기대치에 미치지 못하는 클라이언트 측 병목 현상 확인.
- **해결**: 측정 지표를 DB 엔진 처리 시간에서 직렬화와 네트워크 왕복 시간(RTT)을 포함한 'Client Side Latency'로 재정의하여 병목 구간 규명.
- **결과**: 동기식(Synchronous) 환경에서는 전체 성능이 DB 엔진보다 네트워크 비용에 의해 결정됨을 증명.
