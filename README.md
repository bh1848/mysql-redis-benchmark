...
## 7. 트러블 슈팅
### - Test Isolation
* **문제 상황:** 벤치마크를 반복 실행할 때마다 데이터베이스에 이전 테스트 데이터가 누적되어, `INSERT` 성능이 점차 저하되거나 PK 중복 에러가 발생하는 문제 확인.
* **원인 분석:** 테스트 실행 시점마다 Clean 환경이 보장되지 않아 실험의 독립성이 훼손됨.
* **해결:** Spring Boot JPA 설정을 `ddl-auto: create`로 적용하여, 애플리케이션 시작 시마다 스키마를 새로 생성하고 초기화하도록 구성.
* **결과:** 매 실행마다 동일한 조건(Empty Table)에서 테스트를 수행할 수 있게 되어, 실험 데이터의 신뢰성을 확보함.

### - 시간 측정 한계
* **문제 상황:** Redis의 단순 조회(`GET`) 성능 측정 시, 다수의 요청이 `0ms`로 기록되어 정확한 평균 지연시간 산출이 어려운 현상 발생.
* **원인 분석:** Java의 `System.currentTimeMillis()`는 운영체제에 따라 해상도 차이가 있으며, 마이크로초(µs) 단위의 빠른 연산을 측정하기엔 정밀도가 부족함.
* **해결 및 향후 과제:** 개별 연산 측정 대신, Batch-wise의 총 소요 시간을 측정하여 평균을 역산하는 방식을 채택하여 오차를 줄임. 향후 `System.nanoTime()` 도입 고려.
* **결과:** 0ms로 유실되는 데이터 없이 전체 Throughput 기반의 유의미한 성능 지표를 도출함.


### - Iterative Execution의 병목 분석
* **문제 상황:** Redis가 In-Memory DB임에도 불구하고, 예상했던 이론적 성능(수십만 OPS) 대비 낮은 처리량이 기록됨.
* **원인 분석:** 현재 구현이 Pipelining이나 `Batch Update`가 아닌, 어플리케이션 레벨의 `for-loop`를 통한 동기식 순차 처리 구조임. 이로 인해 DB 자체 성능보다는 RTT가 전체 성능을 Dominate하고 있음을 파악.
* **해결/의의:** 본 프로젝트의 목적이 'Loop에서의 DB별 기본 응답 속도 비교'임에 집중하여, 구조를 유지하되 RTT를 포함한 애플리케이션 체감 성능을 지표로 삼기로 결정.
* **결과:** 단순히 DB 엔진의 속도가 아니라, 실제 클라이언트가 겪는 Latency 차이를 명확히 시각화함.
